...
#if BASETEXTURE
               in TEX2D_PARAM(BaseTextureSampler),
               in TEX2D_PARAM(LightmapSampler),
#endif
               in TEX2D_PARAM(NormalSampler),
               in TEX2D_PARAM(RefractSampler),
               in TEX2D_PARAM(ReflectSampler),
               out vec4 result, out float fogFactor)
{
...
#if MULTITEXTURE
    vec4 vNormal  = texture(TEX2D(NormalSampler), i.vBumpTexCoord);
    vec4 vNormal1 = texture(TEX2D(NormalSampler), i.vExtraBumpTexCoord.xy);
    vec4 vNormal2 = texture(TEX2D(NormalSampler), i.vExtraBumpTexCoord.zw);
...
#else
    vec4 vNormal = DecompressNormal(TEX2D_ARG(NormalSampler), i.vBumpTexCoord, NORMAL_DECODE_MODE);
#endif
...
#if ABOVEWATER
    float waterFogDepthValue = texture(TEX2D(RefractSampler), unwarpedRefractTexCoord).a;
#else
    ...
    vec4 vReflectColor = texture(TEX2D(ReflectSampler), vReflectTexCoord);
#if BLURRY_REFRACT
    ...
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -2.0 * ddx1 + -2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -2.0 * ddx1 + -1.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -2.0 * ddx1 + 0.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -2.0 * ddx1 + 1.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -2.0 * ddx1 + 2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -1.0 * ddx1 + -2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -1.0 * ddx1 + -1.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -1.0 * ddx1 + 0.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -1.0 * ddx1 + 1.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + -1.0 * ddx1 + 2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 0.0 * ddx1 + -2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 0.0 * ddx1 + -1.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 0.0 * ddx1 + 0.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 0.0 * ddx1 + 1.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 0.0 * ddx1 + 2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 1.0 * ddx1 + -2.0 * ddy1);
    vRefractColor += texture(TEX2D(RefractSampler), vRefractTexCoord + 1.0 * ddx1 + -1.0 * ddy1);
...
#else
    vReflectColor *= i.vReflectTint;
    vec4 vRefractColor = texture(TEX2D(RefractSampler), vRefractTexCoord);
    // get the depth value from the refracted sample to be used for fog.
#	if ABOVEWATER
    // Don't mess with this in the underwater case since we don't really have
    // depth values there.
    waterFogDepthValue = texture(TEX2D(RefractSampler), vRefractTexCoord).a;
#	endif
#endif
...
#if BASETEXTURE
    vec4 baseSample = texture(TEX2D(BaseTextureSampler), i.vBumpTexCoord.xy);
    vec2 bumpCoord1;
    vec2 bumpCoord2;
    vec2 bumpCoord3;
    ...
    vec4 lightmapSample1 = texture(TEX2D(LightmapSampler), bumpCoord1);
    vec3 lightmapColor1 = lightmapSample1.rgb;
    vec3 lightmapColor2 = texture(TEX2D(LightmapSampler), bumpCoord2).rgb;
    vec3 lightmapColor3 = texture(TEX2D(LightmapSampler), bumpCoord3).rgb;
