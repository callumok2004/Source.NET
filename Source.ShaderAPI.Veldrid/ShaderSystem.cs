...
public class ShaderSystem : IShaderSystemInternal
{
	...
	public void DrawElements(IShader shader, IMaterialVar[] parms, IShaderShadow renderState, VertexCompressionType vertexCompression, uint materialVarTimeStamp) {
		...
		if ((Config.ShowNormalMap || Config.ShowMipLevels == 2) && (IsFlag2Set(parms, MaterialVarFlags2.LightingBumpedLightmap) || IsFlag2Set(parms, MaterialVarFlags2.DiffuseBumpmappedModel))) {
			...
			ref BasePerMaterialContextData? contextData = ref ((ShadowStateVeldrid)renderState).ContextData;
	...
	private void InitState(IShader shader, IMaterialVar[] shaderParams, ref IShaderShadow renderState) {
		...
		shader.DrawElements(shaderParams, renderState, null, VertexCompressionType.None, ref ((ShadowStateVeldrid)renderState).ContextData);
		DoneWithShaderDraw();
	}
	...
	public void AttachProgramCache(VeldridShaderProgramCache cache) {
		programs = cache;
		vshs.Clear();
		pshs.Clear();
	}

	public VeldridShaderProgramCache? Programs => programs;
	...
	private byte[]? BuildShaderSource(ReadOnlySpan<byte> source, ReadOnlySpan<char> defines, string mainName, List<string> sourceFiles) {
		...
		sb.Append("#define SOURCE_VULKAN 1\n");
	...
	private nint CompileShader(VdShaderStages stage, ReadOnlySpan<char> name, ReadOnlySpan<char> defines, string typeName) {
		if (programs == null)
			return 0;
		...
		byte[] source = new byte[handle.Stream.Length];
		handle.Stream.ReadExactly(source);

		List<string> sourceFiles = [];
		string shaderName = new(name);
		byte[]? built = BuildShaderSource(source, defines, shaderName, sourceFiles);
		if (built == null)
			return 0;

		byte[] spirv;
		try {
			spirv = SourceGlslCompiler.Compile(Encoding.ASCII.GetString(built), shaderName, stage);
		}
		catch (SpirvCompilationException ex) {
			Warning($"WARNING: {typeName} shader compilation error in {shaderName}.\n");
			Warning(ex.Message);
			Warning("\n");
		...
		Dictionary<int, string> samplerNames = RecordSamplerBindings(Encoding.ASCII.GetString(built));

		ReadOnlySpan<char> combos = defines.SliceNullTerminatedString();
		if (combos.IsEmpty)
			Msg($"Compiled shader: {shaderName} ({typeName})\n");
		else
			Msg($"Compiled shader: {shaderName} ({typeName}) [{combos}]\n");

		return programs.Register(new CompiledSpirv {
			Bytes = spirv,
			Name = shaderName,
			Stage = stage,
			EntryPoint = "main",
	...
	public readonly Dictionary<string, int> SamplerBindings = [];

	static readonly System.Text.RegularExpressions.Regex samplerDeclaration =
		new(@"layout\(set\s*=\s*1,\s*binding\s*=\s*(\d+)\)\s*uniform\s+\w+\s+(\w+)_tex\s*;",
			System.Text.RegularExpressions.RegexOptions.Compiled);

	Dictionary<int, string> RecordSamplerBindings(string source) {
		Dictionary<int, string> names = [];

		foreach (System.Text.RegularExpressions.Match match in samplerDeclaration.Matches(source)) {
			int binding = int.Parse(match.Groups[1].Value);
			string name = match.Groups[2].Value;
			SamplerBindings[name] = binding / 2;
			names[binding / 2] = name;
		}
	...
	public VertexShaderHandle LoadVertexShader(ReadOnlySpan<char> name, ReadOnlySpan<char> defines = default) {
		...
		nint compiled = CompileShader(VdShaderStages.Vertex, name, defines, "Vertex");
		if (compiled == 0)
			return VertexShaderHandle.INVALID;

		VertexShaderHandle vsh = new(compiled);
		vshs[symbol] = vsh;
		return vsh;
	}
	...
	public PixelShaderHandle LoadPixelShader(ReadOnlySpan<char> name, ReadOnlySpan<char> defines = default) {
		...
		nint compiled = CompileShader(VdShaderStages.Fragment, name, defines, "Pixel");
		if (compiled == 0)
			return PixelShaderHandle.INVALID;

		PixelShaderHandle psh = new(compiled);
		pshs[symbol] = psh;
		return psh;
	}
